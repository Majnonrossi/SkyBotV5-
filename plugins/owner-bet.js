...
async function handler(m, { conn, args }) {
    ...
    if (db.data.users[m.sender].premiumTime < 1) return m.reply('KHUSUS PREMIUM DEKS\nBELI PREMIUM SANA !')
...
handler.before = async m => {
    ...
    try {
        if (/^(✔️|y(es|a))?$/i.test(txt)) {
            let Bot = (Math.ceil(Math.random() * 3)) * 1
            let Kamu = (Math.floor(Math.random() * 10)) * 1
            let status = 'Kalah'
            if (Bot < Kamu) {
                user.money += count * 1
                status = 'Menang'
