...
const foto = 'https://i.ibb.co/pJqrnfV/default.jpg'
...
let handler = async (m, { conn, text, usedPrefix, command }) => {
	...
	let anu = await fetch(`https://api.lolhuman.xyz/api/ktpmaker?apikey=${global.api}&nik=${a || '-'}&prov=${b || '-'}&kabu=${c || '-'}&name=${d || '-'}&ttl=${e || '-'}&jk=${f || '-'}&jl=${g || '-'}&rtrw=${h || '-'}&lurah=${i || '-'}&camat=${j || '-'}&agama=${k || '-'}&nikah=${l || '-'}&kerja=${n || '-'}&warga=${o || '-'}&until=${p || '-'}&img=${out}`)
