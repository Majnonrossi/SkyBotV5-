...
let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
    ...
    try {
        ...
        let randomkamu = `${Math.floor(Math.random() * 10)}`.trim() //hehe Biar Susah Menang :v
        let Aku = (randomaku * 1)
    ...
    conn.reply(m.chat, caption, m, { quoted : fkontak }, { mentions: conn.parseMention(caption) })
    } else if (Aku < Kamu) {
    let caption = `💰 *C A S I N O* 💰\n\n${htjava} *@${m.sender.split("@")[0]}* - [USER]\n┗┅⭑ ${Kamu} Point\n${htjava} *@${nomorbot.split("@")[0]}* - [BOT]\n┗┅⭑ ${Aku} Point\n\n🎉 *WIN* 🎉\nKamu mendapatkan ${count * 2} Money`.trim()
    conn.reply(m.chat, caption, m, { quoted : fkontak }, { mentions: conn.parseMention(caption) })
    global.db.data.users[m.sender].money += count * 2
    } else {
    let caption = `💰 *C A S I N O* 💰\n\n${htjava} *@${m.sender.split("@")[0]}* - [USER]\n┗┅⭑ ${Kamu} Point\n${htjava} *@${nomorbot.split("@")[0]}* - [BOT]\n┗┅⭑ ${Aku} Point\n\n🔖*DRAW* 🔖\nKamu mendapatkan ${count * 1} Money`.trim()
    conn.reply(m.chat, caption, m, { quoted : fkontak }, { mentions: conn.parseMention(caption) })
    global.db.data.users[m.sender].money >= count * 1
            }
        } else conn.reply(m.chat, `Money kamu tidak mencukupi untuk Casino silahkan *.claim* terlebih dahulu!`.trim(), m)
    } catch (e) {
        console.log(e)
        m.reply('Error!!')
        if (DevMode) {
