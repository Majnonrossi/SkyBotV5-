...
let api = '?apikey=SadTeams'

//> Lolhuman
let resl = 'https://api.lolhuman.xyz/api/random2/'
let apil = '?apikey=SadTeams'
...
const sections = [
  ...
  //{title: `${(ch == true ? false : girlsolo) == true ? p:f}` + "G • Girl Solo", rowId: ".nsfw girlsolo"},
    ...
    {title: `${(ch == true ? false : hentai) == true ? p:f}` + "H • HentaiVideo", rowId: ".hentaivid"},
...
case 'uniform':
	     ...
	     if (!isPrems) return conn.sendButton(m.chat, txtprem, botdate, [['ʙᴜʏ ᴘʀᴇᴍɪᴜᴍ', '.premium'],['ᴏᴡɴᴇʀ', '.owner nomor']], m)
          ...
          conn.sendButton(m.chat, tekk, fot, await(await fetch(res + 'uniform' + api)).buffer(), [[next, `${usedPrefix}nsfw ${args[0]}`]],m)
            break
...
case 'yuri':
	     ...
	     if (!isPrems) return conn.sendButton(m.chat, txtprem, botdate, [['ʙᴜʏ ᴘʀᴇᴍɪᴜᴍ', '.premium'],['ᴏᴡɴᴇʀ', '.owner nomor']], m)
          ...
          conn.sendButton(m.chat, tekk, fot, await(await fetch(resl + 'yuri' + apil)).buffer(), [[next, `${usedPrefix}nsfw ${args[0]}`]],m)
            break
